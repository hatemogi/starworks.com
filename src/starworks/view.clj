  ...
  [:section#사용법
   [:div
    ...
    [:p "다운로드 받은 dmg 파일을 열면 위 화면이 보이며, 스타웍스를 /Applications 폴더로 "
     "드래그 드랍하고 실행합니다."]
    [:p "시스템 설정에 따라 '열 수 없습니다'메시지가 보이면, 시스템 환경설정 > 보안 및 개인 정보 보호 탭"
     " > 스타웍스 '확인 없이 열기' 버튼을 클릭하면 실행됩니다."]
    [:p "화면 우상단 시계 옆 상태메뉴에 스타웍스 아이콘이 보이면 잘 실행된 것이고, "
     "그 상태에서 'KT_starbucks'에 연결하면 앱이 작동합니다."]]])
...
(defn done-page
  ...
  (layout [:main [:section#완료
                  [:div
                   ...
                   [:p.text-center.small "(이 화면은 5초 후에 알아서 사라집니다)"]]]]))
(defn not-found
  []
